...
impl WalletV5R1ExternalBody {
    ...
    pub fn from_cell(cell: Arc<Cell>) -> tonutils_tlb::Result<Self> {
...
pub(super) fn load_v5_inner_request(
    slice: &mut Slice,
) -> tonutils_tlb::Result<(Option<OutList>, Option<WalletV5R1ExtendedActionList>)> {
    let out_list = if slice.load_bit()? {
        Some(tonutils_tlb::load_ref_tlb(
            slice,
...
pub(super) fn wallet_stack_entry<'a, E>(
    method: &'static str,
    stack: &'a tonutils_tvm::TvmStack,
    index: usize,
) -> Result<&'a tonutils_tvm::TvmStackEntry, WalletGetMethodError<E>>
where
    ...
    E: std::error::Error + Send + Sync + 'static,
{
    match wallet_stack_entry(method, stack, index)? {
        tonutils_tvm::TvmStackEntry::Int(value) => Ok(value),
...
{
    match wallet_stack_entry(method, stack, index)? {
        tonutils_tvm::TvmStackEntry::Cell(cell) | tonutils_tvm::TvmStackEntry::Slice(cell) => {
