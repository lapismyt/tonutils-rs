...
use tonutils_tvm::deserialize_boc;

#[cfg(feature = "provider")]
use {
    async_trait::async_trait,
    tonutils_contracts::ContractProvider,
    tonutils_tl::{
        BlockIdExt,
        common::{AccountId, Int256},
        response::{AccountState, MasterchainInfo, RunMethodResult, TransactionList},
    },
    tonutils_tvm::{TvmStack, TvmStackEntry},
};
...
impl ContractProvider for WalletGetMockProvider {
    ...
    async fn get_masterchain_info(&mut self) -> Result<MasterchainInfo, Self::Error> {
        Ok(MasterchainInfo {
            ...
            init: tonutils_tl::common::ZeroStateIdExt {
                workchain: -1,
    ...
    async fn get_account_state_typed(
        ...
        _block: BlockIdExt,
        _account: Address,
    ) -> Result<tonutils_liteclient::boc::DecodedAccountState, Self::Error> {
        unimplemented!("wallet get-method helpers do not read account state")
    }
    ...
    async fn get_account_state_simple(
        ...
        _block: BlockIdExt,
        _account: Address,
    ) -> Result<tonutils_liteclient::boc::SimpleAccount, Self::Error> {
        unimplemented!("wallet get-method helpers do not read account state")
    }
...
async fn v5r1_get_method_helpers_route_expected_methods() {
    ...
    assert_eq!(
        ...
        vec![crate::method_name_to_id("seqno")]
    );
    ...
    assert_eq!(
        provider.method_calls,
        vec![crate::method_name_to_id("get_wallet_id")]
    );
    ...
    assert_eq!(
        provider.method_calls,
        vec![crate::method_name_to_id("get_public_key")]
    );
    ...
    assert_eq!(
        provider.method_calls,
        vec![crate::method_name_to_id("is_signature_allowed")]
    );
    ...
    assert_eq!(
        provider.method_calls,
        vec![crate::method_name_to_id("get_extensions")]
    );
}
