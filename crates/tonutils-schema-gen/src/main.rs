...
const SCHEMA_PATHS: &[(&str, SchemaKind)] = &[
    ...
    (
        "crates/tonutils-tlb/src/tlb/schemas/block_phase1.tlb",
        SchemaKind::Tlb,
    ),
];
...
fn render_module(schema: &Schema) -> String {
    ...
    out.push_str("pub const CONSTRUCTORS: &[ConstructorMetadata] = &[\n");
    for constructor in &schema.constructors {
        writeln!(out, "    ConstructorMetadata {{").unwrap();
        writeln!(out, "        name: {:?},", constructor.name).unwrap();
        writeln!(out, "        tag: {:?},", constructor.tag).unwrap();
        writeln!(out, "        result: {:?},", constructor.result).unwrap();
        writeln!(out, "        field_count: {},", constructor.field_count).unwrap();
        out.push_str("    },\n");
    }
    out.push_str("];\n");
    out
