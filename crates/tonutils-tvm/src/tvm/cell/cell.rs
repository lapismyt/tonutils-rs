...
pub(super) fn validate_cell_data(data: Vec<u8>, bit_len: usize) -> Result<(Vec<u8>, usize)> {
    if bit_len > MAX_CELL_BITS {
        bail!(
            "Cell bit length {} exceeds maximum {}",
            bit_len,
...
pub(super) fn parse_exotic_kind(
    data: &[u8],
    bit_len: usize,
    references: &[Arc<Cell>],
...
pub(super) fn parse_pruned_branch(
    data: &[u8],
    bit_len: usize,
    references: &[Arc<Cell>],
) -> Result<ExoticCellKind> {
...
pub(super) fn parse_library_reference(
    data: &[u8],
    bit_len: usize,
    references: &[Arc<Cell>],
) -> Result<ExoticCellKind> {
...
pub(super) fn parse_merkle_proof(
    data: &[u8],
    bit_len: usize,
    references: &[Arc<Cell>],
) -> Result<ExoticCellKind> {
...
pub(super) fn parse_merkle_update(
    data: &[u8],
    bit_len: usize,
    references: &[Arc<Cell>],
) -> Result<ExoticCellKind> {
...
pub(super) fn exotic_level(exotic: &ExoticCellKind, references: &[Arc<Cell>]) -> Result<u8> {
...
/// For a higher-level API with TON-specific convenience methods, see [`Builder`](crate::Builder).
///
...
pub(super) fn shift_left_in_place(bytes: &mut [u8], shift: usize) {
