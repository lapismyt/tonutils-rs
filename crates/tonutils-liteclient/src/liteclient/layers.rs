...
use crate::types::{LiteError, LiteService};
use tonutils_tl::adnl::Message;
use tonutils_tl::common::Int256;
use tonutils_tl::request::{LiteQuery, RawWrappedRequest, WrappedRequest};
use tonutils_tl::response::{Error, Response};
            ...
            match response {
                Message::Answer { answer, .. } => {
                    let response = tl_proto::deserialize(&answer).map_err(|e| {
                        LiteError::TlError(tonutils_tl::TlError::ParseError(e.to_string()))
                    })?;
                    Ok(response)
                }
    ...
    fn call(&mut self, request: Message) -> Self::Future {
        let (query_id, request) = match request {
            Message::Query { query_id, query } => {
                let decoded: Result<LiteQuery, LiteError> =
                    tl_proto::deserialize(&query).map_err(|e| {
                        LiteError::TlError(tonutils_tl::TlError::ParseError(e.to_string()))
                    });
                match decoded {
...
mod tests {
    ...
    use super::*;
    use tonutils_tl::request::LiteQueryRaw;
