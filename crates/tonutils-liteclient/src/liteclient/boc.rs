...
fn simple_account_state(account: &Account) -> SimpleAccountState {
    match account {
        ...
        Account::Full { storage, .. } => match &storage.state {
            tonutils_tlb::AccountState::Uninit => SimpleAccountState::Uninit,
            tonutils_tlb::AccountState::Frozen { .. } => SimpleAccountState::Frozen,
            tonutils_tlb::AccountState::Active { .. } => SimpleAccountState::Active,
        },
    }
}
...
mod tests {
    ...
    fn decoded_account_state_decodes_none_account_state_cell() {
        ...
        assert_eq!(decoded.account, Some(tonutils_tlb::Account::None));
    ...
    fn full_account(last_trans_lt: u64, grams: u64) -> tonutils_tlb::Account {
        full_account_for(Address::new(0, [0x11; 32]), last_trans_lt, grams)
    }

    fn full_account_for(address: Address, last_trans_lt: u64, grams: u64) -> tonutils_tlb::Account {
