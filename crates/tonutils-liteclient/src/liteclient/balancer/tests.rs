...
mod tests {
    ...
    fn mock_client() -> LiteClient {
        LiteClient::from_service(service_fn(
            |request: tonutils_tl::request::RawWrappedRequest| async move {
                ...
                async move {
                    *calls.lock().await += 1;
                    Ok::<_, LiteError>(tl_proto::serialize(
                        tonutils_tl::response::Response::SendMsgStatus(
                            tonutils_tl::response::SendMsgStatus { status: 1 },
                        ),
                    ))
                }
            },
    ...
    fn queued_response_client(
        ...
        outcomes: Vec<std::result::Result<tonutils_tl::response::Response, LiteError>>,
    ) -> LiteClient {
    ...
    fn recording_response_client(
        ...
        response: tonutils_tl::response::Response,
    ) -> LiteClient {
            ...
            move |request: tonutils_tl::request::RawWrappedRequest| {
                ...
                async move {
                    let request = tl_proto::deserialize(&request.request).map_err(|error| {
                        LiteError::TlError(tonutils_tl::TlError::ParseError(error.to_string()))
                    })?;
    ...
    fn block_header_response(seqno: i32) -> tonutils_tl::response::Response {
        tonutils_tl::response::Response::BlockHeader(tonutils_tl::response::BlockHeader {
    ...
    async fn typed_helper_retries_adnl_errors_through_peer_selection_path() {
        ...
        let peers = vec![
            ...
            queued_response_client(
                ...
                vec![Ok(tonutils_tl::response::Response::LibraryResult(
    ...
    async fn lite_server_error_is_not_retried_but_updates_peer_failure_state() {
        ...
        let peers = vec![
            queued_response_client(
                Arc::clone(&peer0_calls),
                vec![Ok(tonutils_tl::response::Response::Error(
                    tonutils_tl::response::Error {
                        code: 400,
            ...
            queued_response_client(
                Arc::clone(&peer1_calls),
                vec![Ok(tonutils_tl::response::Response::CurrentTime(
                    tonutils_tl::response::CurrentTime { now: 1 },
                ))],
            ),
        ];
        let mut balancer = LiteBalancer::new(peers, Duration::from_millis(50));
    ...
    async fn local_typed_decode_error_is_not_retried() {
        ...
        let peers = vec![
            ...
            queued_response_client(
                Arc::clone(&peer1_calls),
                vec![Ok(tonutils_tl::response::Response::LibraryResult(
                    tonutils_tl::response::LibraryResult { result: Vec::new() },
                ))],
            ),
        ];
        let mut balancer = LiteBalancer::new(peers, Duration::from_millis(50));
