...
fn response_client(
    ...
    response: std::result::Result<tonutils_tl::response::Response, LiteError>,
) -> LiteClient {
...
async fn non_retryable_errors_do_not_mark_peer_dead() {
    ...
    balancer
        ...
        .await
        .insert(0, PeerState::Healthy);
    let error = LiteError::TlError(tonutils_tl::TlError::ParseError("bad boc".into()));
...
async fn send_message_attempts_distinct_peers_and_returns_first_success() {
    ...
    let peers = vec![
        ...
        response_client(
            ...
            Ok(tonutils_tl::response::Response::SendMsgStatus(
