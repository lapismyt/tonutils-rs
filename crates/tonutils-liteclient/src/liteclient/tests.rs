...
fn test_lite_error_display_includes_server_details() {
    let err = LiteError::ServerError(tonutils_tl::response::Error {
        code: 400,
...
async fn query_raw_waits_through_limiter_before_calling_service() {
    ...
    let service = service_fn(move |request: tonutils_tl::request::RawWrappedRequest| {
...
fn test_candidate_id() -> tonutils_tl::common::NonfinalCandidateId {
    tonutils_tl::common::NonfinalCandidateId {
...
fn response_client(response: Response) -> super::client::LiteClient {
    ...
    super::client::LiteClient::from_service(service_fn(
        move |_request: tonutils_tl::request::RawWrappedRequest| {
...
fn request_response_client(
    ...
    let bytes = tl_proto::serialize(response);
    super::client::LiteClient::from_service(service_fn(
        move |request: tonutils_tl::request::RawWrappedRequest| {
...
async fn liteclient_basic_methods_build_requests_and_decode_responses() {
    ...
    let mut client = response_client(Response::CurrentTime(tonutils_tl::response::CurrentTime {
        now: 123,
    }));
    assert_eq!(client.get_time().await.unwrap(), 123);

    let mut client = response_client(Response::Version(tonutils_tl::response::Version {
        mode: 1,
        version: 2,
        capabilities: 3,
        now: 4,
    ...
    let mut client = response_client(Response::BlockData(tonutils_tl::response::BlockData {
        id: test_block_id(),
        data: vec![1, 2, 3],
    }));
    assert_eq!(
    ...
    let mut client = response_client(Response::BlockState(tonutils_tl::response::BlockState {
        id: test_block_id(),
    ...
    let mut client = response_client(Response::BlockHeader(tonutils_tl::response::BlockHeader {
        id: test_block_id(),
        mode: (),
    ...
    let stack = client
        .run_get_method_typed(
            0,
            test_block_id(),
            tonutils_tvm::Address::new(0, [0; 32]),
            1,
            tonutils_tvm::TvmStack::empty(),
        )
        .await
        .unwrap();
...
async fn liteclient_extended_methods_cover_flagged_request_paths() {
    let config = tonutils_tl::response::ConfigInfo {
        mode: (),
        id: test_block_id(),
        state_proof: vec![1],
        config_proof: vec![2],
    ...
    let mut client = response_client(Response::ValidatorStats(
        tonutils_tl::response::ValidatorStats {
            mode: (),
            id: test_block_id(),
            count: 1,
            complete: true,
    ...
    let cell_boc =
        tonutils_tvm::serialize_boc(&tonutils_tvm::Builder::new().build().unwrap(), false).unwrap();
    let mut client = response_client(Response::LibraryResult(
        tonutils_tl::response::LibraryResult {
            result: vec![
                LibraryEntry {
                    hash: Int256([1; 32]),
                    data: Vec::new(),
    ...
    let raw_libraries = tonutils_tl::response::LibraryResultWithProof {
        id: test_block_id(),
        mode: (),
    ...
    let mut client = response_client(Response::ShardBlockProof(
        tonutils_tl::response::ShardBlockProof {
            masterchain_id: test_block_id(),
            links: vec![tonutils_tl::response::ShardBlockLink {
                id: test_block_id(),
                proof: vec![5],
            }],
        },
    ...
    let mut client = response_client(Response::OutMsgQueueSizes(
        tonutils_tl::response::OutMsgQueueSizes {
            shards: vec![tonutils_tl::response::OutMsgQueueSize {
                id: test_block_id(),
                size: 9,
            }],
    ...
    let mut client = response_client(Response::BlockOutMsgQueueSize(
        tonutils_tl::response::BlockOutMsgQueueSize {
            mode: (),
            id: test_block_id(),
            size: 11,
            proof: Some(vec![6]),
    ...
    let mut client = response_client(Response::DispatchQueueInfo(
        tonutils_tl::response::DispatchQueueInfo {
            mode: (),
            id: test_block_id(),
            account_dispatch_queues: vec![tonutils_tl::response::AccountDispatchQueueInfo {
                addr: Int256([4; 32]),
                size: 12,
                min_lt: 1,
                max_lt: 2,
    ...
    let mut client = response_client(Response::DispatchQueueMessages(
        tonutils_tl::response::DispatchQueueMessages {
            mode: (),
            id: test_block_id(),
            messages: Vec::new(),
            complete: false,
...
async fn liteclient_nonfinal_methods_decode_responses() {
    let mut client = response_client(Response::NonfinalValidatorGroups(
        tonutils_tl::response::NonfinalValidatorGroups {
            groups: vec![tonutils_tl::response::NonfinalValidatorGroupInfo {
                ...
                candidates: vec![tonutils_tl::common::NonfinalCandidateInfo {
    ...
    let mut client = response_client(Response::NonfinalCandidate(
        tonutils_tl::response::NonfinalCandidate {
            id: id.clone(),
            data: vec![1],
            collated_data: vec![2],
        },
    ...
    let mut client = response_client(Response::NonfinalPendingShardBlocks(
        tonutils_tl::response::NonfinalPendingShardBlocks {
...
async fn query_raw_request_timeout_maps_to_lite_error_timeout() {
    let service = service_fn(
        move |_request: tonutils_tl::request::RawWrappedRequest| async move {
...
async fn query_raw_without_request_timeout_preserves_existing_behavior() {
    let service = service_fn(
        move |request: tonutils_tl::request::RawWrappedRequest| async move {
...
async fn query_typed_decodes_success_response_and_rejects_unexpected_type() {
    ...
    let decoded: tonutils_tl::response::MasterchainInfo = client
    ...
    let error = client
        .query_typed::<tonutils_tl::response::MasterchainInfo>(Request::GetMasterchainInfo)
        .await
        .unwrap_err();
...
async fn lookup_block_builds_request_and_decodes_block_header_response() {
    ...
    let response = tonutils_tl::response::BlockHeader {
        id: test_block_id(),
        mode: (),
        with_state_update: Some(()),
    ...
    assert_eq!(
        request,
        Request::LookupBlock(tonutils_tl::request::LookupBlock {
            mode: (),
            id: block,
            seqno: Some(()),
            lt: Some(11),
...
async fn lookup_block_with_proof_builds_request_and_decodes_result() {
    ...
    let response = tonutils_tl::response::LookupBlockResult {
        id: test_block_id(),
        mode: (),
    ...
    assert_eq!(
        request,
        Request::LookupBlockWithProof(tonutils_tl::request::LookupBlockWithProof {
            mode: (),
            id,
            mc_block_id,
            lt: Some(33),
...
async fn raw_get_block_decodes_block_boc() {
    use tonutils_tlb::{Block, TlbSerialize};
    ...
    let mut client = response_client(tonutils_tl::response::Response::BlockData(
        tonutils_tl::response::BlockData { id, data },
    ));
...
async fn get_account_state_typed_extracts_simple_account() {
    ...
    use tonutils_tvm::{Address, serialize_boc};

    let account = Account::None;
    let state = serialize_boc(&account.to_cell().unwrap(), false).unwrap();
    let id = test_block_id();
    let mut client = response_client(tonutils_tl::response::Response::AccountState(
        tonutils_tl::response::AccountState {
            id: id.clone(),
            shardblk: id,
            shard_proof: Vec::new(),
            proof: Vec::new(),
...
async fn run_get_method_typed_rejects_nonzero_exit_code() {
    ...
    let id = test_block_id();
    let mut client = response_client(tonutils_tl::response::Response::RunMethodResult(
        tonutils_tl::response::RunMethodResult {
            mode: (),
            id: id.clone(),
            shardblk: id,
