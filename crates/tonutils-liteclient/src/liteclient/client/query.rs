...
impl LiteClient {
    ...
    pub async fn connect_config(config: &ConfigGlobal, liteserver_index: usize) -> Result<Self> {
        let liteserver = config
            ...
            .map_err(|e| LiteError::TlError(tonutils_tl::TlError::ParseError(e.to_string())))?;
    ...
    pub async fn raw_get_block(&mut self, id: BlockIdExt) -> Result<tonutils_tlb::Block> {
    ...
    pub async fn raw_get_block_header(
        ...
        let header_proof =
            crate::boc::DecodedBoc::decode(&raw.header_proof).map_err(decode_error)?;
    ...
    pub async fn raw_get_account_state(
        ...
        Option<tonutils_tlb::Account>,
        Option<tonutils_tlb::ShardAccount>,
    )> {
    ...
    pub async fn run_get_method_typed(
        ...
        if result.exit_code != 0 {
            return Err(LiteError::TlError(tonutils_tl::TlError::ParseError(
                format!("run get method exited with code {}", result.exit_code),
            )));
        }
        let stack = result
            .result
    ...
    pub async fn raw_get_shard_info(
        ...
        let shard_descr = crate::boc::ShardDescr {
            boc: crate::boc::DecodedBoc::decode(&raw.shard_descr).map_err(decode_error)?,
        };
        Ok(DecodedShardInfo {
            raw,
            shard_proof,
    ...
    pub async fn raw_get_all_shards_info(
        ...
        let data = crate::boc::DecodedBoc::decode(&raw.data).map_err(decode_error)?;
    ...
    pub async fn get_all_shards_info_typed(
        ...
        Err(LiteError::TlError(tonutils_tl::TlError::ParseError(
    ...
    pub async fn get_one_transaction_typed(
        ...
        account: AccountId,
        lt: u64,
    ) -> Result<Option<tonutils_tlb::Transaction>> {
        Ok(self
    ...
    pub async fn get_one_transaction_decoded(
        ...
        let transaction = if raw.transaction.is_empty() {
            ...
            Some(crate::boc::decode_transaction_boc(&raw.transaction).map_err(decode_error)?)
        };
    ...
    pub async fn raw_get_block_transactions_ext(
        ...
        reverse_order: bool,
        want_proof: bool,
    ) -> Result<Vec<tonutils_tlb::Transaction>> {
        Ok(self
