...
impl Cli {
    ...
    pub(super) fn execute_schema(&self, command: &SchemaCommand) -> Result<()> {
        match command {
            SchemaCommand::Check => {
                let generated = tonutils_tlb::schema::generate_block_phase1()?;
                let constructors =
                    tonutils_tlb::schema::parse_schema(tonutils_tlb::schema::BLOCK_PHASE1_TLB)?;
                let view = SchemaCheckView {
                    schema: "block_phase1.tlb",
                    constructors: constructors.len(),
                    generated_matches: generated == tonutils_tlb::schema::BLOCK_PHASE1_GENERATED,
                };
    ...
    pub(super) fn execute_boc(&self, command: &BocCommand) -> Result<()> {
        match command {
            BocCommand::Decode {
                ...
                let root = tonutils_tvm::deserialize_boc(&raw).context("failed to decode BoC")?;
        ...
        match command {
            ...
            LiteClientCommand::RunGetMethodTyped {
                ...
                let method_id = method_id
                    .or_else(|| method.as_deref().map(tonutils_contracts::method_name_to_id))
            ...
            WalletCommand::Seqno { address } => {
                ...
                let result = backend
                    .run_get_method(
                        ...
                        tonutils_contracts::method_name_to_id("seqno"),
                        TvmStack::empty(),
                    )
                    .await
