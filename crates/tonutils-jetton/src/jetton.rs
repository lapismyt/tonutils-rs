...
use std::sync::Arc;
use thiserror::Error;
use tonutils_metadata::{MetadataError, Tep64Content, Tep64Field, Tep64KnownKey, Tep64Value};
use tonutils_tlb::{MsgAddress, MsgAddressExt, MsgAddressInt, TlbDeserialize, ensure_empty};
use tonutils_tvm::{Address, Cell, Slice, TvmStack, TvmStackEntry};
...
pub fn decode_jetton_master_data(
    ...
    let jetton_content = tonutils_metadata::parse_tep64_content(stack_entry_cell(&stack, 3)?)?;
...
pub trait JettonContractExt {
    type Error: std::error::Error + Send + Sync + 'static;

    /// Runs `get_jetton_data` at the provider's latest masterchain block and
    /// decodes the returned TEP-74 stack.
    async fn jetton_master_data_latest(
        &mut self,
    ) -> Result<JettonMasterData, tonutils_contracts::ContractError<Self::Error>>;

    /// Runs `get_jetton_data` and returns parsed jetton metadata fields.
    async fn jetton_metadata_latest(
        &mut self,
    ) -> Result<JettonMetadata, tonutils_contracts::ContractError<Self::Error>>;
}

#[cfg(feature = "provider")]
impl<'a, P: tonutils_contracts::ContractProvider + ?Sized> JettonContractExt
    for tonutils_contracts::Contract<'a, P>
{
    type Error = P::Error;

    async fn jetton_master_data_latest(
        &mut self,
    ) -> Result<JettonMasterData, tonutils_contracts::ContractError<P::Error>> {
        let stack = self
            .run_get_method_by_name_typed_latest("get_jetton_data", TvmStack::empty())
            .await?;
        decode_jetton_master_data(stack).map_err(tonutils_contracts::ContractError::decode)
    }

    async fn jetton_metadata_latest(
        &mut self,
    ) -> Result<JettonMetadata, tonutils_contracts::ContractError<P::Error>> {
...
mod tests {
    use super::*;
    use tonutils_metadata::{Tep64Value, tep64_key_hash};
    use tonutils_tlb::{MsgAddress, TlbSerialize};
    use tonutils_tvm::{BitKey, Builder, HashmapE};
    ...
    mod provider_tests {
        ...
        use tonutils_contracts::{Contract, ContractError, ContractProvider};
        use tonutils_liteclient::boc::{DecodedAccountState, SimpleAccount};
        use tonutils_tl::{
            BlockIdExt, Int256,
        ...
        async fn provider_helper_uses_latest_block_and_get_jetton_data_method_id() {
            ...
            assert_eq!(
                ...
                vec![crate::method_name_to_id("get_jetton_data")]
            );
        }
