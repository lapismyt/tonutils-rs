...
pub fn decode_nft_collection_data(
    ...
    let collection_content = tonutils_metadata::parse_tep64_content(stack_entry_cell(
        &stack, 1, METHOD,
    )?)
...
pub fn parse_nft_metadata_cell(cell: Arc<Cell>) -> Result<NftMetadata, NftMetadataError> {
    tonutils_metadata::parse_tep64_content(cell)
...
pub trait NftContractExt {
    type Error: std::error::Error + Send + Sync + 'static;

    /// Runs `get_collection_data` at the provider's latest masterchain block.
    async fn nft_collection_data_latest(
        &mut self,
    ) -> Result<NftCollectionData, tonutils_contracts::ContractError<Self::Error>>;

    /// Runs `get_nft_data` at the provider's latest masterchain block.
    async fn nft_item_data_latest(
        &mut self,
    ) -> Result<NftItemData, tonutils_contracts::ContractError<Self::Error>>;

    /// Runs `get_collection_data` and returns parsed collection metadata.
    async fn nft_collection_metadata_latest(
        &mut self,
    ) -> Result<NftMetadata, tonutils_contracts::ContractError<Self::Error>>;
...
#[cfg(feature = "provider")]
impl<'a, P: tonutils_contracts::ContractProvider + ?Sized> NftContractExt
    for tonutils_contracts::Contract<'a, P>
{
    ...
    async fn nft_collection_data_latest(
        ...
        let stack = self
            ...
            .await?;
        decode_nft_collection_data(stack).map_err(tonutils_contracts::ContractError::decode)
    }
    ...
    async fn nft_item_data_latest(
        ...
        let stack = self
            ...
            .await?;
        decode_nft_item_data(stack).map_err(tonutils_contracts::ContractError::decode)
    }
    ...
    async fn nft_item_metadata_latest(
        ...
        if data.collection_address.is_some() {
            return Err(tonutils_contracts::ContractError::decode(
                NftMetadataError::CollectionContentRequired,
            ));
        }
        parse_nft_metadata_cell(data.individual_content)
            .map_err(tonutils_contracts::ContractError::decode)
    }
