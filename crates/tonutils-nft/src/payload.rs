...
use std::sync::Arc;
use tonutils_jetton::{ForwardPayload, empty_forward_payload, query_id_payload, std_address};
use tonutils_tlb::{CellRef, Grams, MsgAddress, Result, TlbDeserialize, TlbSerialize};
use tonutils_tvm::{Address, Builder, Cell, Slice};
    ...
    pub fn new(query_id: u64, index: impl Into<BigUint>, collection: Option<Address>) -> Self {
        Self {
            ...
            collection: collection
                .map(std_address)
                .unwrap_or(MsgAddress::Ext(tonutils_tlb::MsgAddressExt::None)),
        }
    }
...
pub(crate) fn exact_from_cell<T: TlbDeserialize>(cell: Arc<Cell>) -> Result<T> {
    ...
    tonutils_tlb::ensure_empty(&slice)?;
    Ok(value)
}
...
fn expect_op(slice: &mut Slice, expected: u32, schema: &'static str) -> Result<()> {
    ...
    if actual != expected {
        return Err(tonutils_tlb::TlbError::CustomSchema {
            schema,
