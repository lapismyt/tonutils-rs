...
use tonutils_jetton::{inline_forward_payload, referenced_forward_payload};
use tonutils_tlb::{Either, MsgAddress, MsgAddressExt, TlbDeserialize};
use tonutils_tvm::{Address, Builder, Slice};

fn address(seed: u8) -> Address {
    Address::new(0, [seed; 32])
}

fn comment_cell(text: &str) -> std::sync::Arc<tonutils_tvm::Cell> {
...
fn builds_and_decodes_nft_transfer_payload() {
    ...
    assert_eq!(decoded.new_owner, tonutils_jetton::std_address(address(1)));
    assert_eq!(
        decoded.response_destination,
        tonutils_jetton::std_address(address(2))
    );
...
fn builds_and_decodes_ownership_assigned_payload() {
    ...
    assert_eq!(decoded.prev_owner, tonutils_jetton::std_address(address(3)));
    match decoded.forward_payload {
...
fn builds_and_decodes_report_static_data_payload() {
    ...
    assert_eq!(decoded.collection, tonutils_jetton::std_address(address(4)));
}
...
fn builds_and_decodes_report_royalty_params_payload() {
    ...
    assert_eq!(
        decoded.destination,
        tonutils_jetton::std_address(address(5))
    );
}
