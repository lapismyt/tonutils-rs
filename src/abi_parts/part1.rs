...
pub enum AbiCodecError {
    ...
    /// Stack conversion for this ABI type is intentionally not defined yet.
    #[error("ABI stack conversion for {ty} is unsupported")]
    UnsupportedType {
        /// ABI type family.
        ty: &'static str,
    },
...
fn ensure_non_empty(field: &'static str, value: &str) -> Result<(), AbiModelError> {
...
fn validate_integer_width(kind: &'static str, bits: u16) -> Result<(), AbiModelError> {
...
fn validate_integer_width_codec(kind: &'static str, bits: u16) -> Result<(), AbiCodecError> {
    if !(1..=ABI_INTEGER_MAX_BITS).contains(&bits) {
...
fn ensure_signed_range(value: &BigInt, bits: u16) -> Result<(), AbiCodecError> {
...
fn ensure_unsigned_range(value: &BigUint, bits: u16) -> Result<(), AbiCodecError> {
...
fn snake_cell(bytes: &[u8]) -> Result<Arc<Cell>, AbiCodecError> {
...
fn read_snake_cell(cell: Arc<Cell>) -> Result<Vec<u8>, AbiCodecError> {
    let mut bytes = Vec::new();
    read_snake_cell_into(cell, &mut bytes)?;
    Ok(bytes)
}

fn read_snake_cell_into(cell: Arc<Cell>, bytes: &mut Vec<u8>) -> Result<(), AbiCodecError> {
...
fn address_slice_cell(address: &Address) -> Result<Arc<Cell>, AbiCodecError> {
...
fn decode_address_slice(cell: Arc<Cell>) -> Result<Address, AbiCodecError> {
    let mut slice = Slice::new(cell);
    let address =
...
fn abi_type_name(ty: &AbiType) -> &'static str {
    match ty {
        AbiType::Int { .. } => "int",
        AbiType::Uint { .. } => "uint",
        AbiType::Bool => "bool",
...
fn abi_value_name(value: &AbiValue) -> &'static str {
    match value {
        AbiValue::Int(_) => "int",
