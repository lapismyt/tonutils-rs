...
use crate::liteclient::{layers::{UnwrapErrorLayer, WrapMessagesLayer}, peer::LitePeer, types::LiteError};
...
impl LiteClient {
    ...
    pub async fn run_smc_method(&mut self, mode: u32, block: BlockIdExt, account: Address, method_id: u16, params: Vec<u8>) -> Result<RunMethodResult> {
        let request = Request::RunSmcMethod(RunSmcMethod { mode: mode, id: block, account: account.to_account_id(), method_id: method_id as u32, params });
        let response: RunMethodResult = self.send_request(request).await?;
        Ok(response)
    }

    pub async fn get_shard_info(&mut self, block: BlockIdExt, workchain: i32, shard: u64, exact: bool) -> Result<ShardInfo> {
        let request = Request::GetShardInfo(GetShardInfo { id: block, workchain, shard, exact });
    ...
    pub async fn get_one_transaction(&mut self, block: BlockIdExt, account: AccountId, lt: u64) -> Result<TransactionInfo> {
        let request = Request::GetOneTransaction(GetOneTransaction { id: block, account, lt });
